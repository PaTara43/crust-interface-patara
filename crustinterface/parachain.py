"""
Tool for interacting with Crust Polkadot Parachain.
...
class Parachain(Base):
    """
    Class made for interacting with Crust Polkadot Parachain to store files via Web3 services. Most of the queries
        and extrinsics should be performed in Crust Main net with Mainnet, since files via Polkadot are actually
        stored in Crust Mainnet.
    """

    def __init__(self, seed: tp.Optional[str] = None, crypto_type: int = KeypairType.SR25519):

        super().__init__(chain="parachain", seed=seed, crypto_type=crypto_type)
